...
if __name__ == "__main__":
    ...
    p = [
        Profile().add("support", min=(1, 80, 90), opt=(1, 80, 90), max=(1, 80, 90)),
        Profile().add("query", min=(1, 16, 90), opt=(1, 16, 90), max=(1, 16, 90)),
        Profile().add("labels", min=(1, 5), opt=(1, 5), max=(1, 5))
    ]
    i = {"image": np.ones(shape=(1, 80, 90), dtype=float),
         "query": np.ones(shape=(1, 16, 90), dtype=float),
         "labels": np.ones(shape=(1, 5), dtype=int)}
    create_engine(p,
                  'modules/ar/trx/checkpoints/trx_1_80_90_static.onnx',
                  'modules/ar/trx/checkpoints/trx_1_80_90_static.engine',
                  i)
